...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.amero", new NodeConfig());
			node.start();
			...
			ameroClient ec = node.getameroClient();
	...
	// signatures too. Regression test for go-amero issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/amero/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.amero", "github.com/amero/go-amero/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "amero", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
