...
import (
	...
	"github.com/amero/go-amero/accounts/usbwallet/internal/trezor"
...
// or whether the amero app was not started on it.
...
// amero address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.ameroAddress)
	if _, err := w.trezorExchange(&trezor.ameroGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.ameroTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.ameroTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
