...
// NewProtocolManager returns a new amero sub protocol manager. The amero sub protocol manages peers capable
// with the amero network.
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light amero protocol")
	...
	log.Info("Light amero protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light amero peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light amero handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light amero peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light amero message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light amero message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the amero sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // amero network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
