...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the amero protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for amero and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // amero address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an amero light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // amero protocol stack
	client *ethclient.Client   // Client connection to the amero chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the amero light client protocol
...
// close terminates the amero connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the amero address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/amero/go-amero/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and amero address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for amero addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No amero address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the amero address and profile URL.
...
// returning the username, avatar URL and amero address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain amero address,
