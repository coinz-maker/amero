...
// nodeDockerfile is the Dockerfile required to run an amero node.
var nodeDockerfile = `
FROM amero/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.amero/keystore/ && cp /signer.json /root/.amero/keystore/' >> geth.sh && \{{end}}
...
// an amero node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.amero{{if .Ethashdir}}
...
// deployNode deploys a new amero node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.amero"],
